...
const WalletTransactionsHistory: FC<WalletTransactionsHistoryProps> = ({ allTransactions = [], processingTransaction }) => {
  const renderTransactionHistory = () => {
    return (
      <>
        <div className={`wallet-onboarding h-[calc(100vh_-_305px)] overflow-y ease-in-out transition-all duration-1000 ${allTransactions.length > 0 ? ' fixed opacity-0 invisible translate-y-[200px]' : 'visible'}`}><WalletOnboarding /></div>
